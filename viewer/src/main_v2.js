...
app.innerHTML = `
  ...
  <main class="comparison">
    <section class="panel">
      ...
      <div id="scene-a" class="scene"></div>
    </section>
    ...
    <section class="panel">
      ...
      <div id="scene-b" class="scene"></div>
    </section>
  </main>
...
async function start() {
  ...
  function animate(now) {
    ...
    if (playing) {
      simulationTime += realDelta * PLAYBACK_SPEED;

      if (simulationTime >= maxDuration) {
        simulationTime = maxDuration;
        playing = false;
        playPauseButton.textContent = "Play";
      }
    }
    ...
    if (simulationTime < divergenceTime) {
      statusDisplay.textContent =
        `shared trajectory · ${Math.min(
          stateA.activeCount,
          stateB.activeCount
        )} tokens`;
    } else {
      statusDisplay.textContent =
        `diverged at token ${divergenceIndex} · ` +
        `A ${stateA.activeCount} · B ${stateB.activeCount}`;
    }
